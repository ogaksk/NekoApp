using UnityEngine;
using UnityEngine.Audio;
using System.Collections;
using UnityEngine.UI;


public class AudioTest : MonoBehaviour {
  public AudioMixer mixer;
  const double PI = System.Math.PI;
  const double PI2 = PI * 2.0;
  const double PI_2 = PI / 2.0;
  const double sampling_frequency = 48000;
  const double PI2_SR = PI2 / sampling_frequency;
  private enum PlayState {
    Stop,
    SineWave,
    SquareWave,
    TriangleWave,
    SawtoothWave
  }

  public double frequency = 200;
  public double gain = 0.5;
  private double increment;
  private double time;
  private PlayState playState = PlayState.Stop;
  private float FetchVal = 0f;
  public Text PhaseVal;
  public Text PitchVal;

	// Use this for initialization
	void Start () {
	 
   playState = PlayState.SawtoothWave;
	}
	
	// Update is called once per frame
	void Update () {
    if (Input.GetMouseButton(0))
    {
      mixer.SetFloat("Frequency", 2.0f + 15.0f * Mathf.Pow(Input.mousePosition.x / Screen.width, 3.0f));
      frequency =30 + 300 * Mathf.Pow(Input.mousePosition.y / Screen.height, 3.0f);
      // mixer.SetFloat("Radius2", 0.1f + 2.0f * Mathf.Pow(Input.mousePosition.y / Screen.width, 3.0f));
      // mixer.SetFloat("Period", 15.0f + 20.0f * Mathf.Pow(Input.mousePosition.y / Screen.width, 3.0f));
      Camera.main.backgroundColor = new Color(Camera.main.backgroundColor.r, Input.mousePosition.y / Screen.height, Input.mousePosition.x / Screen.width, 1.0f);
    }

    bool result =  mixer.GetFloat("Frequency", out FetchVal);
      if(result){
        PhaseVal.text = FetchVal.ToString();
      }else{
        PhaseVal.text = "none";
    }
    PitchVal.text = frequency.ToString();
	}



  void OnAudioFilterRead(float[] data, int channels)
  {
    switch (playState) {
      case PlayState.SineWave:
        SineWave(data, channels);
        break;
      case PlayState.SquareWave:
        SquareWave(data, channels);
        break;
      case PlayState.TriangleWave:
        TriangleWave(data, channels);
        break;
      case PlayState.SawtoothWave:
        SawtoothWave(data, channels);
        break;
    }
  }

  void SineWave(float[] data, int channels) {
    increment = frequency * 2 * PI / sampling_frequency;
    for (var i = 0; i < data.Length; i = i + channels) {
      time = time + increment;
    
      data [i] = (float)(gain * System.Math.Sin (time));
      if (channels == 2)
        data [i + 1] = data [i];
      if (time > 2 * System.Math.PI)
        time = 0;
    }
  }

  void SquareWave(float[] data, int channels) {
    increment = frequency * 2 * PI / sampling_frequency;
    for (var i = 0; i < data.Length; i = i + channels) {
      time = time + increment;
    
      data [i] = (float)(gain * ((time % PI2) < PI2 * 0.5 ? 1.0 : -1.0));
      if (channels == 2)
        data [i + 1] = data [i];
      if (time > 2 * System.Math.PI)
        time = 0;
    }
  }
  
  void TriangleWave(float[] data, int channels) {
    increment = frequency * 2 * PI / sampling_frequency;
    for (var i = 0; i < data.Length; i = i + channels) {
      time = time + increment;
      if (time > 2 * System.Math.PI)
        time = 0;
    
      double t = (time + PI_2) % PI2;
      data [i] = (float)(gain * ((t < PI ? t - PI : PI - t) / PI_2 + 1.0));
      if (channels == 2)
        data [i + 1] = data [i];
    }
  }
  
  void SawtoothWave(float[] data, int channels) {
    increment = frequency * 2 * PI / sampling_frequency;
    for (var i = 0; i < data.Length; i = i + channels) {
      time = time + increment;
    
      data [i] = (float)(gain * ((time + PI) % PI2) / PI - 1.0);
      if (channels == 2)
        data [i + 1] = data [i];
      if (time > 2 * System.Math.PI)
        time = 0;
    }
  }

  void OnGUI ()
  {
    int y = 10;
    if (GUI.Button (new Rect (10, y, 100, 30), "サイン波")) {
      playState = PlayState.SineWave;
      gain = 1.0;
    }
    y += 40;
    if (GUI.Button (new Rect (10, y, 100, 30), "矩形波")) {
      playState = PlayState.SquareWave;
      gain = 0.1;
    }
    y += 40;
    if (GUI.Button (new Rect (10, y, 100, 30), "三角波")) {
      playState = PlayState.TriangleWave;
      gain = 1.0;
    }
    y += 40;
    if (GUI.Button (new Rect (10, y, 100, 30), "ノコギリ波")) {
      playState = PlayState.SawtoothWave;
      gain = 0.1;
    }
    y += 40;
    if (GUI.Button (new Rect (10, y, 100, 30), "Stop")) {
      playState = PlayState.Stop;
    }
  }


}

